...
\begin{itemize}[parsep=0.5ex]
      ...
      \item \en{Implemented OpenResty gateway plugin for developers to call the log report (support for field desensitization, log structure normalization), access to Shopee data platform to provide data source support for other functions}
            \zh{实现了 OpenResty 网关插件,用于开发者调用日志上报(支持字段脱敏、日志结构规范化),接入 Shopee 数据平台为其它功能提供数据源支持}
\end{itemize}
