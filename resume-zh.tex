...
\begin{itemize}
      \item \en{The main work is the maintenance and optimization of the internal quality testing platform  (Go), as well as being responsible for the development of the internal testing platform and its version change (Python)}
            \zh{负责效能部门内部质量检测平台的维护（Go），以及内部测试平台的开发及其版本优化（Python）}
      \item \en{}
...
\en{}
\zh{C++ 实现 LSM-Tree 数据结构}
\begin{itemize}
      \item \en{}
            \zh{cLSM 采用分层结构，In-memory Buffer 和 Disk-Backed Storage 底层都采用 SkipList 结构组成，查找时从上到下，并且利用 BloomFilter 降低读放大问题。}
      \item \en{}
            \zh{利用堆优化 Merge 操作；删除则利用墓碑机制标记 Key，之后查找和 Merge 会忽略此 Key。}
      \item \en{}
            \zh{cLSM 利用批量存储技术规避磁盘随机写入问题，相比 B+ Tree 牺牲读性能，大幅提高写性能。}
\end{itemize}
...
\begin{itemize}
      \item \en{}
            \zh{Cache 单元采用 LRU 缓存策略，上层封装实现单机缓存和基于 HTTP 的分布式缓存，使用一致性哈希选择节点（负载均衡），利用 SingleFlight 机制防止缓存击穿。}
      \item \en{}
            \zh{对比 MemCached：goCache 无需设置服务端，并且利用 SingleFlight 机制减少请求次数。}
\end{itemize}
...
\begin{itemize}[parsep=0.25ex]
      ...
      \item \en{\textbf{System}: }
            \zh{\textbf{系统}: 进程相关包括通信、同步、调度；了解死锁以及虚拟内存等；熟悉 Socket 编程，进程间通讯编程；熟悉 Linux I/O 模型，了解多路复用机制。}
      ...
      %       \zh{\textbf{分布式系统}: 有分布式系统开发经验，熟悉 Raft 算法，参加过 MIT6.824，PingCAP Talent Plan 等分布式课程。}

      \item \en{\textbf{Developing Tools}: }
            \zh{\textbf{开发工具}: 熟悉 Linux，有 Git 等团队合作工具的经验。}
\end{itemize}
...
\begin{itemize}[parsep=0.25ex]
      ...
      \item \en{}
            \zh{开源爱好者，喜欢折腾，热爱编程，喜欢记录以及分享技术，写博客}
      % \item \en{}
